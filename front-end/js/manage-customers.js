...
btnSave.on('click', () => {
    ...
    xhr.addEventListener('readystatechange', ()=> {
        if (xhr.readyState === 4){
            ...
            if (xhr.status === 201){
                ...
                modalElm.modal("hide");
                // txtName.trigger('focus');
                showToast('success', 'Updated', 'Customer has been updated successfully');
            } else{
    ...
    if (btnSave.text() === "Save Customer") {
        ...
        xhr.open('PATCH', `${REST_API_BASE_URL}/customers/${txtId.val()}`, true);
...
tbodyElm.on('click', ".edit", (eventData) => {
    ...
    modalElm.find("h1").text("Update Customer");


    let trElm = $(eventData.target).parents("tr");
    txtId.val(+$(trElm).children("td:first-child").text().replace('C', ''));
    txtName.val($(trElm).children("td:nth-child(2)").text());
    txtAddress.val($(trElm).children("td:nth-child(3)").text());
    txtContact.val($(trElm).children("td:nth-child(4)").text());
...
$("#btn-new-customer").on('click', () => {
    ...
    modalElm.find("h1").text("Add new Customer");
});
